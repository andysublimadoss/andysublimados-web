  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-8 bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl shadow-slate-200/20">
        ...
        <div className="flex items-center bg-slate-50 rounded-[2rem] p-2 border border-slate-100 shadow-inner">
          ...
          <span className="px-8 font-black text-slate-700 capitalize min-w-[200px] text-center text-lg tracking-tight">{monthName}</span>
                ...
                {monthlyMovements.sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((m, idx) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      key={m.id} 
                      className="grid grid-cols-12 items-center p-6 bg-slate-50/50 hover:bg-white hover:shadow-xl hover:shadow-slate-200/40 rounded-[2rem] border border-transparent hover:border-slate-100 transition-all group"
                    >
