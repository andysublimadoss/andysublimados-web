...
const CustomersList: React.FC<CustomersListProps> = ({ customers, setCustomers, orders, showToast, onNavigate }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 md:gap-8 bg-white p-6 md:p-10 rounded-3xl md:rounded-[3.5rem] border border-slate-100 shadow-xl shadow-slate-200/20">
        ...
        <div className="relative w-full lg:w-112.5">
          ...
          <input 
            ...
            className="w-full pl-12 md:pl-16 pr-6 md:pr-8 py-4 md:py-5 bg-slate-50 border border-slate-100 rounded-[1.5rem] md:rounded-4xl outline-none shadow-inner focus:ring-4 focus:ring-indigo-500/5 font-bold text-sm transition-all placeholder:text-slate-300"
          />
        </div>
      </div>

      <div className="bg-white rounded-3xl md:rounded-[4rem] shadow-2xl shadow-slate-200/40 border border-slate-50 overflow-hidden">
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-0 md:p-4 z-110 overflow-y-auto">
            ...
            >
              ...
              <div className="relative h-32 md:h-48 bg-indigo-600 shrink-0">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,var(--tw-gradient-stops))] from-indigo-500 via-indigo-600 to-indigo-800 opacity-90" />
                <div className="absolute -bottom-12 md:-bottom-16 left-6 md:left-12 flex items-end gap-4 md:gap-8">
                  <div className="w-24 h-24 md:w-40 md:h-40 bg-white rounded-3xl md:rounded-5xl p-1.5 md:p-2 shadow-2xl border-4 border-white">
                    <div className="w-full h-full bg-indigo-50 text-indigo-600 rounded-[1.5rem] md:rounded-4xl flex items-center justify-center">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pt-16 md:pt-24 px-6 md:px-12 pb-12">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
                  ...
                  <div className="lg:col-span-4 space-y-8 md:space-y-10">
                    ...
                    {/* Notas del Cliente */}
                    <div className="bg-white p-6 md:p-8 rounded-4xl border border-slate-100 shadow-xl shadow-slate-200/20">
                          ...
                          return sortedProducts.length > 0 ? sortedProducts.map(([name, count], idx) => (
                            <div key={name} className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] md:text-xs font-bold text-slate-700 truncate max-w-30">{name}</span>
                              </div>
                    ...
                    <div className="space-y-4">
                      {orders.filter(o => o.customerName === selectedCustomer.name).length > 0 ? (
                        orders
                          ...
                          .map((order) => (
                            <div key={order.id} className="group bg-white hover:bg-slate-50 rounded-[1.5rem] md:rounded-4xl p-5 md:p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
                              ...
                              <div className="flex flex-col items-end gap-2 md:gap-3 w-full md:w-auto">
                                <div className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-[8px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 md:gap-2 ${
                                  ...
                                  {order.status === 'terminado' ? <CheckCircle2 size={10} /> : <Clock size={10} />}
                                  {order.status}
                                </div>
                        ...
                        <div className="text-center py-12 md:py-24 bg-slate-50 rounded-5xl border border-dashed border-slate-200">
              ...
              <div className="p-6 md:p-12 bg-slate-50/80 backdrop-blur-md border-t border-slate-100 flex justify-end shrink-0">
                <button 
                  onClick={() => setIsHistoryModalOpen(false)}
                  className="w-full md:w-auto px-12 md:px-16 py-4 md:py-6 bg-slate-900 text-white rounded-2xl md:rounded-4xl font-black uppercase tracking-widest text-[10px] md:text-sm hover:bg-black transition-all active:scale-95 shadow-2xl"
                >
                  Cerrar Perfil
                </button>
              </div>
        ...
        {isEditModalOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-110 overflow-y-auto">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 30 }}
              className="bg-white rounded-5xl w-full max-w-xl overflow-hidden shadow-2xl flex flex-col"
            >
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <textarea 
                      ...
                      className="w-full pl-16 pr-8 py-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 font-bold outline-none focus:ring-4 focus:ring-indigo-500/5 text-slate-700 text-sm transition-all min-h-30" 
                      placeholder="Ej: Prefiere remeras de algodón, siempre paga en efectivo..." 
                    />
                  </div>
                </div>

                <div className="p-6 md:p-8 bg-amber-50 rounded-3xl border border-amber-100 flex gap-4 md:gap-6">
        ...
        {isDeleteModalOpen && customerToDelete && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-120">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 30 }}
              className="bg-white rounded-5xl w-full max-w-md overflow-hidden shadow-2xl p-8 md:p-12 text-center"
            >
